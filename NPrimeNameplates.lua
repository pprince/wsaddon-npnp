...
local _typeColor =
{
    Self        = _color("88" .. "a3cf59"),
    Friendly    = _color("ff" .. "547d0f"), -- Friendly NPC Text Color
    FriendlyP   = _color("ff" .. "7daf29"), -- Friendly Player Text Color
    Neutral     = _color("ff" .. "e6a639"), -- Neutral Text / Bar Color ?
    Hostile     = _color("ff" .. "b82828"), -- Hostile NPC
    HostileP    = _color("ff" .. "ffd080"), -- Hostile player Text
    HostilePPVP = _color("ff" .. "ff0000"), -- Hostile player PvP Flagged Text
    Group       = _color("ff" .. "a3cf59"), -- Group Bar color.
    LowThreat   = _color("ff" .. "ff80ca"), -- Threat Loss Bar Color.
    CC          = _color("ff" .. "7e00ff"), -- MoO Bar Color
    LowHP       = _color("ff" .. "b2b2b2"), -- Low HP Threash hold
    Cleanse     = _color("ff" .. "af40e1"), -- Cleanse Bar Color.
    Harvest     = _color("ff" .. "b2b2b2"),
    Other       = _color("ff" .. "ffffff"), -- No idea
    Hidden      = _color("ff" .. "ffffff"),
}
...
function NPrimeNameplates:UpdateNameplateColors(p_nameplate, p_ccID, p_unit)
        ...
        local l_barColor    = _typeColor[p_nameplate.type]

        local l_isHostile   = p_nameplate.disposition == Unit.CodeEnumDisposition.Hostile
        local l_isFriendly  = p_nameplate.disposition == Unit.CodeEnumDisposition.Friendly
        local l_isSelf      = p_nameplate.type == "Self"
        ...
        -- Player Characters
            ...
            if (l_isHostile) then
                ...
                if (l_pvpFlagged) then
                        l_textColor = _typeColor["HostilePPVP"]
                        l_barColor = l_textColor
                else
                ...
                if (l_group and not l_isSelf) then
                        l_textColor = _typeColor["Group"]
                        l_barColor = _textColor
                end
        ...
        -- NPCs
            ...
            if (l_isHostile and l_aggroLost) then
                    l_barColor  = _typeColor["LowThreat"]
                    l_textColor = _typeColor["LowThreat"]
            end
...
function NPrimeNameplates:GetColorFlags(p_nameplate)
        ...
        if (_matrix["ConfigAggroIndication"]) then
            if (p_nameplate.inCombat and not p_nameplate.isPlayer) then
                if (p_nameplate.unit:GetTarget() ~= _player) then
...
function NPrimeNameplates:GetUnitType(p_unit)
  ...
  if (l_type == "NonPlayer" and not p_unit:GetUnitRaceId())  then
    ...
    -- if (not l_level) thens
    --  return "Hidden"   --- prevents nameplates on lattice's Obstinate Logic Wall adds
    -- end
  end
